...
export default function PedidoVendaFromLead({ dadosIniciais, onSuccess, onCancel }: PedidoVendaFromLeadProps) {
  ...
  const salvarPedido = async () => {
    ...
    try {
      ...
      console.log('✅ Pedido criado com sucesso, chamando onSuccess...')
      toast.success("Pedido criado com sucesso!")
      
      // Aguardar um momento antes de chamar onSuccess
      await new Promise(resolve => setTimeout(resolve, 500))
      
      // Chamar onSuccess que irá marcar o lead como ganho e fechar o modal
      onSuccess()
    } catch (error: any) {
                  ...
                  {showParceirosDropdown && parceiros.length > 0 && (
                    <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg max-h-48 overflow-auto">
                      {parceiros.map((p) => (
                        ...
                        >
                          <div className="font-medium">{p.NOMEPARC}</div>
                          <div className="text-xs text-muted-foreground">{p.CGC_CPF}</div>
                        </div>
                      ))}
                    </div>
                  )}
